...
int main(int argc, char *argv[]) {
  // Quick check
  assert(query_test(1 << 10, 8192, 4096, 20, 1) == 0);
}
...
int query_test(uint64_t num_items, uint64_t item_size, uint32_t degree,
  ...
  std::stringstream s;
  int query_ser_size = client.generate_serialized_query(index, s);
  cout << "!!!!!!!!!!!Size of query: "<< query_ser_size << endl;
  ...
  //Get reply size:
  int reply_sz = server.serialize_reply(reply, s);
  cout << "!!!!!Reply size: " << reply_sz << endl;
